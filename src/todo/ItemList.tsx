        ...
        {items && (  //gud . gud. ok
          <IonList>
            {items && booksShow.map((item: ItemProps) =>
              ...
              <Item key={item._id} _id={item._id} name={item.name}  onEdit={id => history.push(`/item/${id}`)} author={item.author} available={item.available} publish_date={item.publish_date} pages={item.pages} photoBase64={item.photoBase64} latitude={item.latitude} longitude={item.longitude}/>
            )}
          </IonList>
